...
// 목표:
// 1. map 메서드 사용학생 이름 (name) 과 평균 점수 (averageScore) 값을 가진 새로운 배열 생성
...
const addAverageScore = s => { return {name : s.name, average : calcAverage(s)}};
const newStudents = students.map(addAverageScore);

console.log("=== 이름과 평균 점수 배열 ===");
console.log(students.map(addAverageScore));
